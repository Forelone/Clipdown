...
public class GunScript : MonoBehaviour
{
    ...
    public void TryShoot()
    {
        ...
        if (RG.transform.parent != null && RG.transform.parent.TryGetComponent(out Rigidbody RGP) && RGP.isKinematic == false) 
        {
            RGP.AddForce(Mathf.Clamp(BulletOnBarrel.Spd,0,200) * -transform.forward);
            RGP.AddTorque(BulletOnBarrel.Spd / 2 * -transform.right);
        }
